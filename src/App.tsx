...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'tracker' | 'add-plan'>('tracker');
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-800 flex flex-col justify-between">
      ...
      <div className="flex-grow">
        ...
        {/* Auth splash view if not authenticated */}
        {needsAuth ? (
              ...
              <div className="max-w-md mx-auto space-y-6">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left space-y-3.5">
                  ...
                  <div className="flex items-start space-x-3 text-xs text-slate-600">
                    <CheckCircle size={15} className="text-indigo-600 shrink-0 mt-0.5" />
                    <span>
                      <strong className="text-slate-950 font-bold">จัดเรียงวาระประจำวันให้อัตโนมัติ:</strong> หากคุณมีหัวข้อย่อยหรือมาตราหลายฉบับที่ต้องทบทวนตรงกันในวันเดียวกัน ระบบจะรวมกลุ่มจัดระเบียบให้เรียบร้อยเพื่อการเรียนรู้ในครั้งเดียว
                    </span>
                  </div>
                  <div className="flex items-start space-x-3 text-xs text-slate-600">
                    <CheckCircle size={15} className="text-indigo-600 shrink-0 mt-0.5" />
                    <span>
                      <strong className="text-slate-950 font-bold">ถอนโครงสร้างแบบ Cascade แสนฉลาด:</strong> เมื่อกดลบหรือยกเลิกแผนการเรียนรู้ใดๆ ระบบจะย้อนล้างข้อมูลเฉพาะส่วนของแผนนั้นใน Google Calendar ของคุณ โดยไม่กระทบมาตราอื่นๆ ที่บันทึกอยู่คู่กัน
                    </span>
                  </div>
                </div>
            ...
            {currentView === 'tracker' ? (
              <div className="max-w-5xl mx-auto space-y-6">
                <Dashboard
                  ...
                  token={token}
                  onNavigateToAddPlan={() => setCurrentView('add-plan')}
                />
              </div>
            ) : (
